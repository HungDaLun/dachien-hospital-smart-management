...
2. **Runtime Engine**: Google Gemini 3 API (gemini-3-flash, gemini-3-pro) (File Search & Gems Concept) with Multi-Model Adapter Design
...
*   **F-05 智慧治理建議 (Smart Upload w/ Human-in-the-Loop)**：
    *   **流程**：User 拖曳檔案 -> AI 預填 Metadata -> 彈出確認視窗 -> User 確認 -> 存入資料庫。
    *   **Metadata Trinity (元數據鐵三角)**：
        1.  **Who**: 自動鎖定 `uploaded_by` (User ID)。
        2.  **Where**: 自動繼承 User 的 `department_id` (如「財務部」)，建立資料圍籬。
        3.  **What**: AI 預判 `category` (如「會議記錄」)，使用者可從 **受控詞彙表 (Controlled Vocabulary)** 下拉選單中修正。
        4.  **Naming (命名規範)**：
            *   系統強制執行 `[Dept]-[Type]-[Subject]-[Suffix]` 命名格式。
            *   `Dept` 必須來自系統部門代碼 (`departments.code`)。
            *   `Type` 必須來自文件類別 (`document_categories`)。
            *   AI 僅能建議 `Subject` 與 `Suffix`。
    *   **價值**：確保所有入庫資料都有完美的結構化標籤，實現「設計即治理 (Governance by Design)」。
...
### 4.5 系統管理與主數據 (Admin & MDM) 【NEW】
*   **F-21 統一分類表管理 (Unified Taxonomy)**：
    *   提供後台介面管理 `Document Categories` (文件類別)。
    *   支援階層式結構：Level 1 (業務領域) -> Level 2 (文件類型) -> Level 3 (功能/流程，選填)。
    *   此分類表將作為全系統下單選單的來源 (Single Source of Truth)。
    *   **標準分類架構**：
        *   **Level 1**: 10個業務領域（治理與合規、策略與規劃、人力資源、財務管理、營運管理、行銷與業務、研發與創新、資訊科技、客戶服務、採購與供應鏈）
        *   **Level 2**: 60個文件類型（每個領域下 5-7 個子分類）
        *   **Level 3**: 企業自訂（選填）
    *   **設計原則**：
        *   基於 MECE 原則（相互獨立，完全窮盡）
        *   適用於 95% 的企業
        *   支援行業特定擴展
    *   **詳細方法論請參考**：`docs/TAXONOMY_METHODOLOGY.md`
*   **F-22 部門代碼管理**：
    *   `departments` 表新增 `code` 欄位 (如 `FIN`, `HR`, `IT`)，用於系統內部邏輯判斷與自動化路由。

### 4.6 企業大腦戰情室 (Brain Dashboard) 【NEW】
...
### 核心變更：Metadata Trinity 與動態框架
*   **新增**: `document_categories` 表 (Master Data)。
*   **修改**: `departments` 表新增 `code`。
*   **修改**: `files` 表完善 metadata 欄位，並透過 Tag 系統擴充多維度 Metadata。

**EAKAP 知識治理 (Metadata Strategy):**
*   **Metadata (Agent Retrieval)**: 儲存於 `file_tags` 表，格式為 `Key:Value`。
    *   `Product Context`: `Origins`, `Mega-Mushroom`
    *   `Target Audience`: `Internal`, `Public`
    *   `Governance`: `Approved`, `Draft`
...
ALTER TABLE files ADD COLUMN department_id UUID REFERENCES departments(id); -- 【NEW】部門歸屬，利用 RLS 做圍籬
ALTER TABLE files ADD COLUMN category_id UUID REFERENCES document_categories(id); -- 【NEW】文件類別 (What)

-- 主數據：文件類別 (Unified Taxonomy)
-- 基於企業知識分類方法論 v1.0
-- 標準架構：10個業務領域（Level 1）+ 60個文件類型（Level 2）+ 企業自訂（Level 3）
TABLE document_categories (
  id UUID PRIMARY KEY,
  name VARCHAR(50) NOT NULL,      -- e.g., "治理與合規", "政策", "標準作業程序"
  parent_id UUID,                 -- 支援階層（NULL = Level 1, 有值 = Level 2/3）
  description TEXT,               -- 分類說明與範例
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- 修改部門表，增加代碼
ALTER TABLE departments ADD COLUMN code VARCHAR(20) UNIQUE; -- e.g., "FIN", "HR"
```
