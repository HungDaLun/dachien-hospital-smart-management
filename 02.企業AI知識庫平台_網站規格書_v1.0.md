...
graph TD
    ...
    User[內網使用者] -->|http://ai-brain.local| Next
...
graph TD
    subgraph "組織層級"
        ORG[組織] --> DEPT_A[部門 A]
        ORG --> DEPT_B[部門 B]
        DEPT_A --> TEAM_A1[小組 A1]
        DEPT_A --> TEAM_A2[小組 A2]
    end
...
sequenceDiagram
    ...
    participant Worker as 背景 Worker
    participant Gemini as Gemini API

    U->>FE: 選擇檔案上傳
    FE->>FE: 前端格式驗證
    FE->>API: 上傳請求 (Multipart)
    API->>S3: 儲存原始檔案
    API->>API: 建立 files 記錄 (state: PENDING)
    API-->>FE: 回傳上傳成功

    Note over Worker: 非同步處理
    Worker->>S3: 讀取原始檔案
    Worker->>Worker: 格式轉換 (if needed)
    Worker->>Worker: 品質檢測
    alt 品質通過
        Worker->>Gemini: 上傳至 Gemini File API
        Gemini-->>Worker: 回傳 google_file_uri
        Worker->>API: 更新 state: SYNCED
    else 品質不通過
        Worker->>API: 更新 state: NEEDS_REVIEW
        Worker->>API: 記錄問題報告
    end
```
...
```mermaid
graph TD
    A[發送 Gemini API 請求] --> B{回應成功}
    B -->|是| C[正常處理回應]
    B -->|否| D{錯誤類型}
    
    D -->|429 Rate Limit| E[等待後重試]
    D -->|500/502/503| F[切換備援區域]
    D -->|401/403| G[通知管理員]
    D -->|Timeout| H[重試 2 次]
